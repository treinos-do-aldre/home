...
const TagContent = () => {
    ...
    const { tags, loading, refreshTags, actionId, setActionId} = useContext(CrudContext);
    ...
    const onDelete = async (item: any) => {
        ...
        const res = await service.customRequest("tag", "delete", `id=${item.id}`, {})
        refreshTags();
    }
    ...
    const onCreate = async () => {
        if(!loadingSubmit) {
            ...
            let r = "Tag " + (Math.random() + 1).toString(36).substring(2);
            const res = await service.customRequest("tag", "post", "", {name: r})
            refreshTags();
    ...
    return (
        <StyledComponent theme={theme}>
            ...
            <Table columns={columns} data={tags} deleteFunc={onDelete} editFunc={onEdit} actionId={actionId} />
